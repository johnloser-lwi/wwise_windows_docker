...
const inputGtkFile = "gtk.scss";
const outputGtkFile = "gtk.css";
...
(async () => {
    try {
        ...
        for await (const file of klaw(".")) {
            ...
            if (filename === inputSassFile) {
                tasks.push(sassToJson(file.path, path.join(directory, outputJsonFile)));
            } else if (filename === inputGtkFile) {
                tasks.push(compileSass(file.path, path.join(directory, outputGtkFile)));
            }
        }
...
function compileSass(inputFile: string, outputFile: string) {
...
function sassToJson(inputFile: string, outputFile: string) {
    return new Promise<void>((resolve, reject) => {
        console.log(`Compiling ${inputFile}`);
        sass.render({
            file: inputFile,
            ...
            const cssString: string = String(result.css);
            ...
            await writeFile(outputFile, stringify(jsonObject, { space: 4 }), 'utf-8');
            resolve();
        });
    });
}
...
async function refreshWwise() {
    try {
        ...
        const session = await waapi.connect('ws://localhost:8080/waapi');
        await session.call(ak.wwise.ui.commands.execute, {command:"ReloadCurrentTheme"});
        await session.disconnect();
        console.log("Theme reloaded");
    } catch (err) {
        if(err.message === "Session closed: unreachable"){
            console.log(`Wwise is not running. Skipping theme reload.`);            
        }
        else if (err.message) {
            console.log(`Could not reload theme in Wwise: '${err.message}'`);
        } else {
            console.log(`Could not reload theme in Wwise: ${JSON.stringify(err, null, 4)}`);
        }
    }
}
