...
static const int MAX_HARMONICS_PACKED = MAX_HARMONICS/4;
...
float order
	: register(c[0]);
float4 sphericalMeterData[MAX_HARMONICS_PACKED]
	: register(c[4]);
float4x4 WorldViewProj
	: register(c[4 + MAX_HARMONICS_PACKED]) : WorldViewProjection;
	...
	)
{
	...
	float sphericalMeterDataRaw[MAX_HARMONICS] = (float[MAX_HARMONICS])sphericalMeterData;
	for (i = 0; i < MAX_HARMONICS; ++i)
	{
		intensity += harmonics[i] * sphericalMeterDataRaw[i];
	}
...
VSOutput mainVS(float4 pos : POSITION0, float4 normal : NORMAL)
{
	...
	float intensity = ProjectSN3DphericalHarmonics(z, -x, y, (int)order);
